...
class SchottkyMonitor():
    ...
    def track(self, particles):
        ...
        z_terms = np.empty((self.n_taylor, len(tau)), dtype=np.cdouble)
    ...
    def process_spectrum(self, inst_spectrum_len, deltaQ, band_width, Qx, Qy, 
        ...
        for region in region_to_process:
            ...
            alpha = np.empty((self.n_taylor,n_freq), dtype=np.cdouble)
            ...
            for i in range(len(self.instantaneous_PSDs[region]), n_inst_spectra):
                ...
                # Seclecting the coefficient (x, y, or z) needed to calculate the i^th instataneous Schottky spectra
    ...
    def _check_Taylor_approx(self):
        ...
        if self.processing_param['z']:
            ...
            max_error = self.N_macropart_max * (delta_omega_max*self.tau_max)**self.n_taylor * \
                        np.exp(delta_omega_max*self.tau_max) / sp.special.factorial(self.n_taylor)
            if np.sqrt(min(self.PSD_avg['center'])) < 100 * max_error:
        ...
        if self.processing_param['x']:
            ...
            max_error = self.N_macropart_max * self.x_max * (delta_omega_max*self.tau_max)**self.n_taylor * \
                        np.exp(delta_omega_max*self.tau_max) / sp.special.factorial(self.n_taylor)
            if np.sqrt(min(self.PSD_avg['upperH'])) < 100 * max_error:
        ...
        if self.processing_param['y']:
            ...
            max_error = self.N_macropart_max * self.y_max * (delta_omega_max*self.tau_max)**self.n_taylor * \
                        np.exp(delta_omega_max*self.tau_max) / sp.special.factorial(self.n_taylor)
            if np.sqrt(min(self.PSD_avg['upperV'])) < 100 * max_error:
